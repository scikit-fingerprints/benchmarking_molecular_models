...
from prepare_data_old import FragmentDataset, CombinedDataset, MoleculeDownstreamDataset, combine_mol_frag_collate
...
if __name__ == '__main__':
    ...
    mol_dataset = MoleculeDownstreamDataset("chem_dataset/{}".format(dataset_name), dataset = dataset_name)

    frag_dataset = FragmentDataset("frag_dataset/{}".format(dataset_name))
    ...
    mol_train_dataset, mol_valid_dataset, mol_test_dataset = scaffold_split(mol_dataset, smiles_list, null_value=0, frac_train=0.8,frac_valid=0.1, frac_test=0.1)
    frag_train_dataset, frag_valid_dataset, frag_test_dataset = scaffold_split(frag_dataset, smiles_list, null_value=0, frac_train=0.8,frac_valid=0.1, frac_test=0.1)

    train_dataset = CombinedDataset(mol_train_dataset, frag_train_dataset)
    valid_dataset = CombinedDataset(mol_valid_dataset, frag_valid_dataset)
    test_dataset = CombinedDataset(mol_test_dataset, frag_test_dataset)

    num_tasks = train_dataset[0].y.shape[-1]

    trainloader = DataLoader(train_dataset, batch_size=args.batchsize, shuffle=True, collate_fn=combine_mol_frag_collate, num_workers = 8)

    valloader = DataLoader(valid_dataset, batch_size=args.batchsize, shuffle=False, collate_fn=combine_mol_frag_collate, num_workers = 8)

    testloader = DataLoader(test_dataset, batch_size=args.batchsize, shuffle=False, collate_fn=combine_mol_frag_collate, num_workers = 8) 
