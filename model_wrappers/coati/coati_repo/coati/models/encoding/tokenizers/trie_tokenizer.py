...
class TrieTokenizer:
    ...
    def __init__(
        ...
        special_tokens=[],
        side_tasks=True,
        on_unknown="raise", # raise or ignore
    ):
        self.n_seq = n_seq
    ...
    def tokenize_text(
        ...
        try:
            if self.on_unknown == "ignore":
                tore = [
                    self.vocab[T] if T in self.vocab else self.unk_token
                    for T in self.pre_tokenize(text)
                ]
            else:
                tore = [
                    self.vocab[T]
            ...
            if len(tore) > self.n_seq and range_check:
                print(f"Oversized String: {len(tore)}")
                tore = tore[: self.n_seq -1] + [tore[-1]]
